...
def run_ml_app():
	...
	with st.beta_expander("Prediction Result"):
		...
		if prediction == 1:
			...
			pred_proability_score = {"Negative DM Risk":pred_prob[0][0]*100,
			"Positive DM Risk":pred_prob[0][1]*100}
			st.write(pred_proability_score)
		else:
			st.success("Positive Risk - {}".format(prediction[0]))
			pred_proability_score = {"Negative DM Risk":pred_prob[0][0]*100,
			"Positive DM Risk":pred_prob[0][1]*100}
			st.write(pred_proability_score)
